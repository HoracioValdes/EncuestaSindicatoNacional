...
import cl.modelo.Area_funcion;
import cl.modelo.Busqueda_encuesta;
import cl.modelo.Centro_medico;
import cl.modelo.Comuna;
import cl.modelo.Credito_actual_socio;
import cl.modelo.Educacion_socio;
import cl.modelo.Estado_civil_socio;
import cl.modelo.Socio_encuesta;
import cl.modelo.Tipo_cargo;
import cl.modelo.Vivienda_socio;
 ...
 * @author Horacio
 */
public class DaoEncuesta extends Conectar {
    
    public ArrayList<Centro_medico> listarCentroMedico() {
        ArrayList<Centro_medico> lstCentro = new ArrayList();
        Centro_medico centro_medico;
        try {
            //Recuperar una conexión.
            Connection con = this.getConexion();
            //Se genera sentecia select
            String strSQL = "SELECT * FROM CENTRO_MEDICO";
            ...
            while (res.next()) {
                ...
                centro_medico.setPrimary(Integer.parseInt(res.getString("CENTRO_MEDICO")));
                centro_medico.setDescripcion(res.getString("DESCRIPCION"));
                //Se agrega el INGRESO a la lista.
                lstCentro.add(centro_medico);
            }
            con.close();
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(DaoEncuesta.class.getName())
    ...
    public int registrarEncuesta(Socio_encuesta socio_encuesta) {
        ...
        try {
            ...
            Logger.getLogger(DaoEncuesta.class.getName()).log(Level.SEVERE, "Problema registro del Driver", ex);
        } catch (SQLException ex) {
            Logger.getLogger(DaoEncuesta.class.getName()).log(Level.SEVERE, "Error SQL.", ex);
        }
        return cantFilas;
    }

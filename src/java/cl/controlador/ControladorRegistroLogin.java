...
public class ControladorRegistroLogin extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userPath.equals("/registro.do")) {
            //Recuperamos los datos del socio a registrar
            Socio_corporativo socio_corporativo = new Socio_corporativo();
            socio_corporativo.setRut(request.getParameter("txtRut"));
            socio_corporativo.setCorreo_corporativo(request.getParameter("txtCorreo"));
            socio_corporativo.setClave(request.getParameter("txtClave"));

            //Consultar existencia previa del rut
            boolean existencia = false;
            existencia = dao.existenciaPrevia(socio_corporativo.getRut());
            if (existencia) {
            ...
            if (errores.isEmpty()) {
                ...
                int cantFilas = dao.registrarSocio_corporativo(socio_corporativo);

                //Verificar la inserción y enviar mensajes.
                if (cantFilas > 0) {
                    msg = "Usted ha sido registrado";
                } else {
